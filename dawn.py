...
def basic_net(channels, weight,  pool, **kw):
    return OrderedDict([
        ...
        ('layer2',
         dict(
             conv_bn(
                 channels['layer1'], channels['layer2'], **kw), pool=pool)),
        ('layer3',
         dict(
             conv_bn(
                 channels['layer2'], channels['layer3'], **kw), pool=pool)),
        ('pool', nn.MaxPool2d(4)),
        ('flatten', Flatten()),
...
class TSVLogger():
    ...
    def append(self, output):
        epoch, hours, acc = output['epoch'], output[
            'total time'] / 3600, output['test acc'] * 100
...
def main():
    ...
    train_set = list(
        zip(transpose(normalise(pad(dataset['train']['data'], 4))), dataset['train']['labels']))
    ...
    train_batches=Batches(Transform(train_set, train_transforms),
                            batch_size, shuffle=True, set_random_choices=True, drop_last=True)
    test_batches=Batches(
        test_set, batch_size, shuffle=False, drop_last=False)
    lr=lambda step: lr_schedule(step / len(train_batches)) / batch_size
    opt=SGD(trainable_params(model), lr=lr, momentum=0.9,
              weight_decay=5e-4 * batch_size, nesterov=True)
