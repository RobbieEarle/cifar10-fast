...
class XarrayLogger():
    ...
    def append(self, row):
        self.n_epoch += 1
        row = dict_to_xarray(row).expand_dims(
            'epoch').assign_coords(epoch=[self.n_epoch])
...
def build_graph(net):
    ...
    parts = lambda path, pfx: tuple(pfx) + \
        path.parts if isinstance(path, RelativePath) \
        else (path,) if isinstance(path, str) else path
    graph = OrderedDict()
...
def train_epoch(model, train_batches, test_batches, optimizer_step, timer, test_time_in_total=True):
    train_stats, train_time = run_batches(
        model, train_batches, True, optimizer_step), timer()
    test_stats, test_time = run_batches(
        model, test_batches, False), timer(test_time_in_total)
    return {
...
def train(model, optimizer, train_batches, test_batches, epochs,
    ...
    for epoch in range(epochs):
        epoch_stats = train_epoch(model, train_batches, test_batches,
                                  optimizer.step, timer, test_time_in_total=test_time_in_total)
        summary = union(
            {'epoch': epoch + 1, 'lr': optimizer.param_values()['lr'] * train_batches.batch_size}, epoch_stats)
...
def make_pydot(nodes, edges, direction='LR', sep=sep, **kwargs):
    ...
    class Subgraphs(dict):
        ...
        def __missing__(self, path):
            subgraph = pydot.Cluster(sep.join(map(str, path)), label=stub(
                path), style='rounded, filled', fillcolor='#77777744')
...
class DotGraph():
    ...
    def __init__(self, net, size=15, direction='LR'):
        ...
        self.edges = [(src, k, {})
                      for (k, (n, i)) in graph.items() for src in i]
...
def remove_by_type(net, node_type):
    ...
    graph = build_graph(net)
    remap = {k: i[0]
             for k, (v, i) in graph.items() if isinstance(v, node_type)}
